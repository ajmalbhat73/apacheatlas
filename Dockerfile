...
COPY atlas_start.py atlas_config.py /opt/apache-atlas-${VERSION}/bin/

RUN cd /opt/apache-atlas-${VERSION}/bin \
    && patch -b -f < atlas_start.py \
    && patch -b -f < atlas_config.py
...
CMD ["/bin/bash", "/opt/apache-atlas-${VERSION}/bin/atlas_start.py"]
